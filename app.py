...
def SolvePuzzle(PuzzleGrid, Puzzle, Answer):
  ...
  if PuzzleGrid[0][0] != 'X':
    ...
    for item in PuzzleGrid:
      grid_copy.append(item)
      
    grid_copy.pop(0)


    for item in grid_copy:
      col.append(item[int(CellInfo[1])])


    while CellInfo[-1] in grid_copy[int(CellInfo[0])-1][1:] or CellInfo[-1] in col:
      print("Cannot use digit already in cell")
      print("Enter row column digit: ")
      print("(Press Enter to stop)")
      CellInfo = input()
